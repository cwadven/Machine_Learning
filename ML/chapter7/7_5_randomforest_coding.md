...
# ensemble RandomForest 이용할 것 import
...
# RandomForest에 모델 만들기
# n_estimators -> 몇개의 tree를 만들 것인가
# RandomForest은 여러개의 tree 모델을 만들 때, 자기가 스스로
...
```python
# 모델 검증할 때 쓰기 위한 값
clf_labels = ['Decision tree', 'RandomForest']
all_clf = [tree, forest]

# 반복문을 이용해서 검증하기
# 원래는 RandomForest만 해주면 되는데, 얼마나 정확한지 알기 위해서
